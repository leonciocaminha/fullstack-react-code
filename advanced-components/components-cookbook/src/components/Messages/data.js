...
const defineMessage = (participants, numMessages) => {
  ...
  const messages = times(numMessages)(generateMessage)
          .sort((a, b) => moment(a.sentAt).isBefore(moment(b.sentAt)))
  return messages;
}
...
const defineMessages = (count) => {
  ...
  const sorted = messages.sort((a, b) => moment(a.lastActiveAt).isAfter(moment(b.lastActiveAt)))
